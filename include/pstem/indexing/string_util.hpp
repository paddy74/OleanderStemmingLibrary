...
{
    return ((c >= L'A') && (c <= L'Z')) || ((c >= 0xC0) && (c <= 0xD6)) ||
                   ((c >= 0xD8) && (c <= 0xDE))
               ? (c + 32)
               : c;
}
...
inline wchar_t toupper(wchar_t c) { return std::towupper(c); }
...
inline wchar_t * strncat(
    wchar_t * strDest, const wchar_t * strSource, size_t count)
{
...
inline int wctomb(char * s, wchar_t wc) { return std::wctomb(s, wc); }
...
inline int strncmp(
    const wchar_t * string1, const wchar_t * string2, size_t count)
{
 ...
 * @param out The character buffer to write the integer as a string into. This
 *  can be either a char* or wchar_t* buffer.
 * @param length The length of the output buffer (in character count).
 * @return int Return 0 on success, -1 on failure. Will fail if the buffer is
 *  either invalid or not large enough to hold the converted value.
 */
...
inline int itoa(long value, charT * out, const size_t length)
{
    ...
    // Not enough space in the buffer or NULL terminator? Clear out the data
    //  that we copied into it and return failure.
...
/**
 * @brief Determines whether a character is a space, tab, or newline. Also
 *  includes double-width and no break spaces.
 *
 * @tparam T
...
/**
 * @brief Determines whether a character is a hexademical digit (0-9,A-F,a-f).
 *
 * @tparam T
 * @param ch The letter to be analyzed.
 * @return true
...
{
    return (
        iswdigit(wchar_t(ch)) || ((ch >= 0x61 /*'a'*/ && ch <= 0x66 /*'f'*/) ||
                                  (ch >= 0x41 /*'A'*/ && ch <= 0x46 /*'F'*/)));
}

/**
 * @brief Converts string in hex format to int. Default figures out how much of
 *  the string is a valid hex string, but passing a value to the second
 *  parameter overrides this and allows you to indicate how much of the string
 *  to try to convert.
 *
 * @tparam T
 * @param hexStr The string to convert.
 * @param length How much of the string to analyze. The value -1 (the default)
 *  will tell the function to read until there are no more valid hexadecimal
 *  digits.
 * @return int The value of the string as an integer.
 */
template <typename T>
inline int axtoi(const T * hexStr, size_t length = -1)
{
    ...
    {
        hexStr += 2;
        // If they specified a length to read then take into account the 0x
    ...
    if (length == static_cast<size_t>(-1))
    {
        ...
        {
            if (currentPos[0] == 0 ||
                !string_util::is_hex_digit(currentPos[0]))
            {
                break;
            }
        } while (currentPos++);
    ...
    std::unique_ptr<int> digitsDeleter(digits);
    std::memset(digits, 0, sizeof(int) * (length + 1));
    while (strPos < length)
    {
        if (hexStr[strPos] == 0) break;
        // 0-9
        if (is_within<T>(
                hexStr[strPos], common_lang_constants::NUMBER_0,
                common_lang_constants::NUMBER_9))
            digits[strPos] = (hexStr[strPos] & 0x0F);
        // A-F
        else if (
            is_within<T>(
                hexStr[strPos], common_lang_constants::LOWER_A,
                common_lang_constants::LOWER_F) ||
            is_within<T>(
                hexStr[strPos], common_lang_constants::UPPER_A,
...
/**
 * @brief Gets the number of characters in the string pointed to by str, not
 *  including the terminating '\0' character, but at most maxlen. In doing
 *  this, strnlen looks only at the first maxlen characters at str and never
 *  beyond str+maxlen. This function should be used for input that may not be
 *  NULL terminated.
 *
 * @tparam T
 * @param str The string to review.
 * @param maxlen The maximum length of the string to scan.
 * @return size_t The valid length of the string or maxlen, whichever is
 *  shorter.
 */
template <typename T>
...
inline const T * stristr(const T * string, const T * strSearch)
{
    ...
    while (*string)
    {
        ...
        for (i = 0; strSearch[i] != 0; ++i)
        {
            ...
            if (string_util::tolower(strSearch[i]) !=
                string_util::tolower(string[i]))
            {
                ++string;
                break;
            }
 ...
 */
template <typename T>
inline const T * strnistr(
    const T * string, const T * strSearch, const size_t string_len)
{
    ...
    for (size_t i = 0; i < string_len; ++i)
    {
        ...
        for (j = 0; strSearch[j] != 0; ++j)
        {
            ...
            if (string_util::tolower(strSearch[j]) !=
                string_util::tolower(string[i + j]))
            {
                break;
            }
        }
...
/**
 * @brief Search string in reverse for substring. "offset" is how far we are in
 *  the source string already and how far to go back.
 *
 * @tparam T
 * @param string
 * @param search
 ...
 */
template <typename T>
inline const T * strrstr(const T * string, const T * search, size_t offset)
{
    if (!string || !search)
    {
        return NULL;
...
/**
 * @brief Natural order comparison functions. Compare, recognizing numeric
 *  strings.
 *
 * @tparam T
 * @param first_string
 ...
 */
template <typename T>
inline int strnatordcmp(
    const T * first_string, const T * second_string,
    bool case_insensitive = false)
{
    ...
    while (true)
    {
        ...
        if (iswdigit(ch1) && iswdigit(ch2))
        {
            ...
            double firstDouble = string_util::strtod(
                first_string + first_string_index, &firstEnd);
            double secondDouble = string_util::strtod(
                second_string + second_string_index, &secondEnd);
            ...
            else  // Numbers are equal
            {
                ...
                // The first string is done, but there is more to the second
                //  string after the number, so first is smaller
                else if (*firstEnd == 0)
                {
                    return -1;
                }
                // The second string is done, but there is more to the first
                //  string after the number, so first is bigger
                else if (*secondEnd == 0)
                {
                    return 1;
                }
                // There is more to both of them, so move the counter and move
                //  on
                else
                {
...
/**
 * @brief Indicates whether a larger strings ends with the specified suffix.
 *  Lengths are provided by the caller for efficiency. This function is case
 *  sensitive.
 *
 * @tparam T
 * @param text
 ...
 */
template <typename T>
inline bool has_suffix(
    const T * text, const size_t text_length, const T * suffix,
    const size_t suffix_length)
{
    ...
    return (
        string_util::strnicmp(
            text + (text_length - suffix_length), suffix, suffix_length) == 0);
}
 ...
 * @return const T* A pointer to where the closing tag is, or NULL if one can't
 *  be found.
 */
template <typename T>
inline const T * find_matching_close_tag(
    const T * string, const T openSymbol, const T closeSymbol,
...
/**
 * @brief Searches for a matching tag, skipping an extra open/close pairs of
 *  symbols in between.
 *
 * @tparam T
 * @param string
    ...
    const T * string, const T * openSymbol, const T * closeSymbol)
{
    ...
    while (*string)
    {
        // To prevent unnecessary calls to strncmp, we check the current
        //  character first
            ...
            string_util::strncmp(string, openSymbol, openSymbolLength) == 0)
        {
            ...
            string[0] == closeSymbolFirstCharacter &&
...
/**
 * @brief Searches for a single character in a string for n number of
 *  characters. Size argument should be less than or equal to the length of the
 *  string being searched.
 *
 * @tparam T
 * @param string The string to search in.
 * @param ch The character to search for.
 * @param numberOfCharacters The number of characters to search through in the
 *  string.
 * @return const T* A pointer in the string where the character was found, or
 * NULL if not found.
 */
template <typename T>
inline const T * strnchr(
    const T * string, const T ch, size_t numberOfCharacters)
{
    if (!string)
    {
        return NULL;
    ...
    {
        // If string being searched is shorter than the size argument then
        //  return failure (NULL).
        if (string[i] == 0)
        {
            NON_UNIT_TEST_ASSERT(
                0 && "strnchr: string shorter than size parameter.");
            return NULL;
        }
        if (string[i] == ch)
        {
...
/**
 * @brief Searches for a single character from a sequence in a string and
 ...
 */
template <typename T>
inline const T * strcspn_pointer(
    const T * string1, const T * string2, const size_t string2Length)
{
    ...
    assert(
        (string_util::strlen(string2) == string2Length) &&
...
/**
 * @brief Searches for a single character from a sequence in a string for n
 ...
 * @return size_t The index into the string that the character was found.
 *  Returns the length of the string if not found.
 */
template <typename T>
inline size_t strncspn(
    const T * stringToSearch, const size_t stringToSearchLength,
    const T * searchString, const size_t searchStringLength)
{
    ...
    assert(
        (string_util::strlen(searchString) == searchStringLength) &&
        "Invalid length passed to strncspn().");
    size_t i = 0, j = 0;
    for (i = 0; i < stringToSearchLength; ++i)
    {
        /* If string being searched is shorter than the size argument then
         *  fail. Note that we should pass back the length of the string that
         *  the caller passed in so that they know that it failed, not the
         *  actual position that we stopped on which would be a NULL termintor.
         */
        if (stringToSearch[i] == 0)
        {
            NON_UNIT_TEST_ASSERT(
                0 && "strncspn: string shorter than size parameter.");
            return stringToSearchLength;
        }
...
/**
 * @brief Searches for a single character not from a sequence in a string in
 *  reverse.
 *
 * @tparam T
 * @param string The string to search in.
 * @param search The sequence of characters to skip.
 * @param offset Where to begin the search. If -1, then the reverse search will
 *  begin at the end of the string.
 * @return size_t The position of where the last non-matching character is at,
 *  or -1 if it can't be found.
 */
template <typename T>
inline size_t find_last_not_of(
    const T * string, const T * search,
    size_t offset = std::basic_string<T>::npos)
{
    ...
    while (offset != std::basic_string<T>::npos)
    {
        ...
        // If the whole search string wasn't scanned, then there was a match.
        //  Move on.
        if (search[i] != 0)
        {
            if (offset == 0)
 ...
 * @param ch The character to search for.
 * @param offset The offset in the string to begin the search from. The default
 *  (-1) will begin the search at the end of the string.
 * @return size_t The offset of the found character, or -1 if not found.
 */
template <typename T>
inline size_t find_last_of(const T * string, const T ch, size_t offset = -1)
{
    if (!string)
    {
...
/**
 * @brief Searches for the first occurrence that is not a character from a
 *  sequence in a string for n number of characters and returns zero-based
 *  index if found.
 *
 * @tparam T
 * @param stringToSearch The string to search.
 * @param stringToSearchLength The length of the string being searched.
 * @param searchString The sequence of characters to perform a reverse match
 *  with.
 * @param searchStringLength The length of the search character sequence.
 * @return size_t The index into the string that the character was not found,
 *  or the length of the string if nothing was found that couldn't match the
 *  search string.
 */
template <typename T>
inline size_t find_first_not_of(
    const T * stringToSearch, const size_t stringToSearchLength,
    const T * searchString, const size_t searchStringLength)
{
    if (!stringToSearch || !searchString || stringToSearchLength == 0 ||
    ...
    NON_UNIT_TEST_ASSERT(
        (string_util::strlen(searchString) == searchStringLength) &&
        "Invalid length passed to find_first_not_of().");
    size_t i = 0, j = 0;
    for (i = 0; i < stringToSearchLength; ++i)
    {
        /* If string being searched is shorter than the size argument then
         *  return string length to indicate failure to find anything. Although
         *  this isn't really the index that we stopped in, this will tell the
         *  caller that the search failed. */
        if (stringToSearch[i] == 0)
        {
            NON_UNIT_TEST_ASSERT(
                0 &&
                "find_first_not_of: string shorter than size "
                "parameter.");
            return stringToSearchLength;
        }
        for (j = 0; j < searchStringLength; ++j)
...
template <typename T>
class equal_basic_string_i_compare : public std::binary_function<T, T, bool>
{
public:
    inline bool operator()(const T & a_, const T & b_) const
    {
...
template <typename T>
class less_basic_string_compare : public std::binary_function<T, T, bool>
{
public:
    inline bool operator()(const T & a_, const T & b_) const
    {
...
{
public:
    const char_typeT * operator()(
        const char_typeT * value,
...
public:
    /**
     * @brief Constructor which takes the string to parse and delimiters to
     *  use.
     *
    ...
    string_tokenize(const T & val, const T & delim)
        : m_value(val),
          ...
          m_next_delim(NULL),
          m_delim(delim),
          m_has_more_tokens(true)
    {
        m_start = m_value.c_str();
        m_next_delim = string_util::strcspn_pointer(
            m_start, m_delim.c_str(), m_delim.length());
    }
     ...
     */
    inline bool has_more_tokens() const { return m_has_more_tokens; }
     ...
     *
     * @note This is useful for seeing if there are any delimiters at all when
     *  first loading the string.
     */
    inline bool has_more_delimiters() const { return (m_next_delim != NULL); }

    /**
     * @brief Get the next token object
     *
     * @return T The next token from the original string as a string object.
     *
     * @note Note that empty tokens can be returned if there is proceeding or
     *  trailing delimiters in the string, or if there are repeated delimiters
     *  next to each other.
     */
...
inline void replace_all(
    T & text, Tchar_type text_to_replace, Tchar_type replacement_text)
{
    size_t start = 0;
    while (start != T::npos)
    {
...
inline void replace_all(
    T & text, const Tchar_type * text_to_replace,
    const Tchar_type * replacement_text)
{
    size_t start = 0;
...
template <typename T>
inline void replace_all(
    T & text, const T & text_to_replace, const T & replacement_text)
{
    size_t start = 0;
    while (start != T::npos)
    {
...
/**
 * @brief Strips extraneous spaces/tabs/carriage returns from a block of text
 *  so that there isn't more than one space consecutively.
 *
 * @tparam string_typeT
 * @param Text
 * @return size_t
...
size_t remove_extra_spaces(string_typeT & Text)
{
    ...
    {
        // If this is the first space found after the current word then it's
        //  OK--just leave it
...
size_t remove_blank_lines(string_typeT & Text)
{
    ...
    for (unsigned int i = 0; i < Text.length(); ++i)
    {
        // If this is the first space found after the current word then it's
        //  OK--just leave it
        if (is_either<wchar_t>(static_cast<wchar_t>(Text[i]), 10, 13) &&
            !alreadyHasNewLine)
        {
        ...
        // This is extra space right after another--get rid of it
            ...
            is_either<wchar_t>(static_cast<wchar_t>(Text[i]), 10, 13) &&
            alreadyHasNewLine)
        {
            // Make sure it isn't a Windows \r\n
            if (i && !(Text[i - 1] == 13 && Text[i] == 10))
...
/**
 * @brief Converts strings to double values, but also takes into account ranges
 *  (returning the average). For example, a string like "5-8" will return 6.5.
 *  Hyphens and colons are seen as range separators.
 *
 * @tparam Tchar_type
 * @param nptr
 * @param endptr
...
inline double strtod_ex(const Tchar_type * nptr, Tchar_type ** endptr)
{
    ...
    const Tchar_type * separator =
        string_util::strcspn_pointer<Tchar_type>(nptr, sepStr, 2);
    // If there is no hyphen or there is one but it is at the end then just
    //  call strtod
...
inline bool is_one_of(
    const Tchar_type character, const Tchar_type * char_string)
{
    while (*char_string)
    {
