...
ã and õ are therefore replaced by a~ and o~ in the word, where ~ is a separate
character to be treated as a consonant. And then R2 and RV have the same
definition as in the Spanish stemmer.
...
indicated.
    - eza   ezas   ico   ica   icos   icas   ismo   ismos   ável   ível   ista
istas oso   osa   osos   osas   amento   amentos   imento   imentos   adora
ador   aça~o adoras   adores   aço~es   ante   antes   ância
        - Delete if in R2.
    - logia logias
...
- ada ida ia aria eria iria ará ara erá era irá ava asse esse isse aste este
iste ei arei erei irei am iam ariam eriam iriam aram eram iram avam em arem
erem irem assem essem issem ado ido ando endo indo ara~o era~o ira~o ar er ir
as adas idas ias arias erias irias arás aras erás eras irás avas es ardes erdes
irdes ares eres ires asses esses isses astes estes istes is ais eis íeis aríeis
eríeis iríeis áreis areis éreis ereis íreis ireis ásseis ésseis ísseis áveis
ados idos ámos amos íamos aríamos eríamos iríamos áramos éramos íramos ávamos
emos aremos eremos iremos ássemos êssemos íssemos imos armos ermos irmos eu iu
ou ira iras

If the last step to be obeyed - either step 1 or 2 - altered the word, do
step 3.
...
in RV, delete it, and if preceded by gu (or ci) with the u (or i) in RV, delete
the u (or i). Or if the word ends ç remove the cedilla.
*/
//------------------------------------------------------
...
public:
    ...
    void operator()(string_typeT & text)
    {
        ...
        string_util::replace_all<string_typeT>(
            text, string_typeT(1, common_lang_constants::LOWER_A_TILDE),
            L"a~");
        string_util::replace_all<string_typeT>(
            text, string_typeT(1, common_lang_constants::UPPER_A_TILDE),
            L"A~");
        string_util::replace_all<string_typeT>(
            text, string_typeT(1, common_lang_constants::LOWER_O_TILDE),
            L"o~");
        string_util::replace_all<string_typeT>(
            text, string_typeT(1, common_lang_constants::UPPER_O_TILDE),
            L"O~");
        ...
        string_util::replace_all<string_typeT>(
            text, L"a~",
            string_typeT(1, common_lang_constants::LOWER_A_TILDE));
        string_util::replace_all<string_typeT>(
            text, L"A~",
            string_typeT(1, common_lang_constants::UPPER_A_TILDE));
        string_util::replace_all<string_typeT>(
            text, L"o~",
            string_typeT(1, common_lang_constants::LOWER_O_TILDE));
        string_util::replace_all<string_typeT>(
            text, L"O~",
            string_typeT(1, common_lang_constants::UPPER_O_TILDE));
    }
...
private:
    ...
    void step_1(string_typeT & text)
    {
        ...
        if (stem<string_typeT>::delete_if_is_in_r2(
                text, /*amentos*/ common_lang_constants::LOWER_A,
                     ...
                     common_lang_constants::UPPER_S))
        {
            // NOOP (fall through to branching statement)
                     ...
                     common_lang_constants::LOWER_C_CEDILLA,
                     common_lang_constants::UPPER_C_CEDILLA,
                     common_lang_constants::LOWER_O,
                     common_lang_constants::UPPER_O,
